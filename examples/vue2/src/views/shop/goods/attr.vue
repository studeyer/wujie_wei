<template>
  ...
  >
    ...
    <el-form v-show="hidden == true" ref="form" :model="form" :rules="rules" label-width="80px">
      <el-form-item label="规则名称">
        <el-row :gutter="10">
          <el-col v-for="(item, index) in items" :key="index" :span="5" style="position: relative; margin-right: 6px">
      ...
      >
        <el-row :gutter="13">
          <el-col v-for="(attr, k) in item.detail" :key="attr" :span="3" :name="attr">
      ...
      <el-form-item v-show="hidden == true">
        <el-row :gutter="24">
          <el-col :span="24"
            ><el-button :loading="loading" type="primary" @click="addGoods(true)">生成</el-button></el-col
          >
        </el-row>
      </el-form-item>
          ...
          <el-form-item>
            <el-row :gutter="24">
              <template v-for="(item, index) in attr.detail">
                <el-col :span="3" style="margin-right: 2px"> {{ index }}:{{ item }} </el-col>
              </template>
              <el-col :span="4">
...
export default {
  // editor,
  components: { picUploadTwo, mulpicUpload, Treeselect },
  props: {
    isAttr: {
      type: Boolean,
      required: true,
  ...
  methods: {
    ...
    addGoods(type) {
      ...
      var that = this;
      isFormatAttr(this.form.id, { items: this.items, attrs: this.attrs })
        .then((res) => {
          this.attrs = res;
        })
        .catch((err) => {
    ...
    submit() {
      ...
      this.loading = false;
      setAttr(this.form.id, { items: this.items, attrs: this.attrs })
        .then((res) => {
          this.attrs = res;
